...
const DataStore = {
  data: {
    ...
    endDate: "2025-01-20",
    years: [...Array(118).keys()].map((num) => num + 1905),
    tau: 2 * Math.PI,
    zoomFactor: 10,
    daysDuration: 43757, // this._dateDiff('1905-01-01', '2020-12-31'),∏
  ...
  _parseDestinationsByYear: function (office) {
    let theData = [];
    DestinationsJson.features
      .filter(
        (destination) => destination.properties.position.toLowerCase() == office
      )
      .forEach((destination) => {
        let year = parseInt(
            destination.properties.date_convert.substring(0, 4)
          ),
          region = destination.properties.new_region
            .replace(/ /g, "")
            .toLowerCase(),
          continentData = theData.filter(
            (continent) => continent.key == region
          );
        if (continentData.length == 0) {
          ...
          let continentYearData = continentData[0].values.filter(
    ...
    theData.forEach((regionData) => {
      ...
      this.data.years.forEach((year) => {
        let regionYearData = regionData.values.filter(
          (yearData) => yearData.year == year
        );
    ...
    theData.sort(
      (a, b) => theOrdering.indexOf(a.key) - theOrdering.indexOf(b.key)
    );
  ...
  _parseDestinationsByLocation: function () {
    let destinations = this.data.selectedId
        ? this.getSimplifiedDestinationsForSelected()
    ...
    destinations.forEach((destination) => {
      let lng = destination.geometry.coordinates[0],
        ...
        dataForLocation = theData.filter(
      ...
      if (dataForLocation.length == 0) {
        theData.push({
          ...
          lng: lng,
          cityId: cityId,
          regionClass: destination.properties.new_region
            .replace(/ /g, "")
            .toLowerCase(),
          displayName:
            destination.properties.city + ", " + destination.properties.country,
          searchName: [
            destination.properties.city,
            destination.properties.country,
            destination.properties.new_region,
          ].join(" "),
          visits: [destination.properties],
        });
      } else {
  ...
  _constrainedDate(date) {
    date =
      date.substring(0, 10) > this.data.startDate
        ? date.substring(0, 10)
        : this.data.startDate;
    date =
      date.substring(0, 10) < this.data.endDate
        ? date.substring(0, 10)
        : this.data.endDate;
    return date;
  },
  ...
  setSelectedVisitsFromLatLng: function (lat, lng) {
    let destinationIds = this.getSimplifiedDestinationsForSelected()
      .filter(
        (v) =>
          v.geometry.coordinates[0] == lng && v.geometry.coordinates[1] == lat
      )
  ...
  allPresidentsShown: function () {
    return !this.data.selectedId && this.data.selectedOffice == "president";
  },

  allSOSsShown: function () {
    return !this.data.selectedId && this.data.selectedOffice == "sos";
  },
  ...
  getInspectedLocationIds: function () {
    return this.data.inspectedLocationIds;
  },

  hasInspectedLocation: function () {
    return this.data.inspectedLocationIds.length > 0;
  },

  getSelectedLocationIds: function () {
    return this.data.selectedLocationIds;
  },

  hasSelectedLocation: function () {
    return this.data.selectedLocationIds.length > 0;
  },

  getVisibleLocationIds: function () {
    return this.hasInspectedLocation()
      ? this.getInspectedLocationIds()
      : this.hasSelectedLocation()
        ? this.getSelectedLocationIds()
        : [];
  },

  hasVisibleLocation: function () {
    return this.getVisibleLocationIds().length > 0;
  },

  isSelectedLocation: function () {
    return this.getVisibleLocationIds() == this.getSelectedLocationIds();
  },

  isAVisibleLocation: function (id) {
    return DataStore.getVisibleLocationIds().indexOf(id) !== -1;
  },
  ...
  getDestinationsByYear: function () {
    return this.data.selectedOffice == "president"
      ? this.data.presidentialDestinationsByYear
      : this.data.sosDestinationsByYear;
  },

  getDestinationDetails: function (ids) {
    return !ids
      ? []
      : ids
        .map(
          (id) =>
            this.getSelectedData().visits.filter(
              (v) => v.properties.cartodb_id == parseInt(id)
            )[0]
        )
        .sort((a, b) =>
          a.properties.start_date < b.properties.start_date ? -1 : 1
        );
  },

  getPresidentialTerms: function () {
    return this.getTermsForOffice("p");
  },

  getSOSTerms: function () {
    return this.getTermsForOffice("s");
  },
  ...
  getTermsForOffice: function (office) {
    return travels
      ...
      .map((officeholder, i) => {
        officeholder.startAngle = this.getDateAngle(
          this._constrainedDate(officeholder.took_office)
        );
        officeholder.endAngle = this.getDateAngle(
          this._constrainedDate(officeholder.left_office)
        );
        return officeholder;
      });
  },
  ...
  getDateAngle: function (date) {
    ...
    let adjustedDuration =
        this.data.daysDuration +
        (this.data.zoomFactor - 1) *
          this.getDuration(this.data.selectedId, this.data.selectedOffice),
    ...
    if (this.dateBeforeSelected(date)) {
      return (
        legendAngle / 2 +
        (this._dateDiff(this.data.startDate, date) / adjustedDuration) *
          nonlegendAngle
      );
    }
    // the selected
      ...
      return (
        ...
        (this._dateDiff(
          this.data.startDate,
          this._constrainedDate(this.getSelectedData().took_office)
        ) /
          adjustedDuration +
          (this._dateDiff(
            this._constrainedDate(this.getSelectedData().took_office),
            ...
            this.data.zoomFactor) /
            adjustedDuration) *
          nonlegendAngle
      );
    } else if (
      this.dateAfterSelected(
        date,
        this.data.selectedId,
        this.data.selectedOffice
      )
    ) {
      return (
        legendAngle / 2 +
        (this._dateDiff(
          ...
          (this._dateDiff(
            this._constrainedDate(this.getSelectedData().took_office),
            this.getSelectedData().left_office
          ) *
            this.data.zoomFactor) /
            adjustedDuration +
          this._dateDiff(this.getSelectedData().left_office, date) /
            adjustedDuration) *
          nonlegendAngle
      );
    }
  ...
  getTermsRingAngles: function () {
    return [this.data.tau * 0.075, this.data.tau - this.data.tau * 0.075];
  },

  dateBeforeSelected: function (date) {
    return !this.data.selectedId
      ? true
      : date.substring(0, 10) <=
          this._constrainedDate(
            this.getOfficeholderData(
              this.data.selectedId,
              this.data.selectedOffice
            ).took_office
          );
  ...
  dateDuringSelected: function (date) {
    return !this.data.selectedId
      ? false
      : date >
          this._constrainedDate(
  ...
  dateAfterSelected: function (date) {
    return !this.data.selectedId
      ? false
      : date >
          this._constrainedDate(
  ...
  monthsForSelected: function () {
    let months = [];
    for (
      let year = this.yearsForSelected()[0];
      year <= this.yearsForSelected()[this.yearsForSelected().length - 1];
      year++
    ) {
      for (let monthNum = 1; monthNum <= 12; monthNum++) {
        let month = ("0" + monthNum).slice(-2),
          ...
          lastDate =
            ...
            month +
            "-" +
            new Date(2022, monthNum - 1, 0).getDate();
        if (
          this.dateDuringSelected(firstDate) ||
          this.dateDuringSelected(lastDate)
        ) {
  ...
  isSelectedYear(year) {
    return this.yearsForSelected().indexOf(year) !== -1;
  },

  getDuration: function (id, office) {
    return !id ? 0 : this._dateDiff(...this.getTerm(id, office));
  },

  getTerm: function (id, office) {
    return [
      this._constrainedDate(this.getOfficeholderData(id, office).took_office),
      this._constrainedDate(this.getOfficeholderData(id, office).left_office),
    ];
  },

  getDestinationsForSelected: function () {
    return this._parseDestinationsByLocation();
  },

  getSimplifiedDestinationsForSelected: function () {
    return this.getSimplifiedDestinationsForOfficeholder(
      this.data.selectedId,
      this.data.selectedOffice
    );
  },
  ...
  getSimplifiedDestinationsForOfficeholder: function (id, office) {
    return !id
      ...
      : travels.filter(
        (officeholder) =>
          officeholder.number == parseInt(id) &&
            officeholder.office == office.substring(0, 1)
      )[0].visits;
  },
    ...
    let destinations = this.getSimplifiedDestinationsForSelected(),
      nextId =
        destinations.findIndex(
          (destination) =>
            destination.properties.cartodb_id ==
            this.data.selectedLocationIds[0]
        ) + 1;
    return destinations[nextId]
      ? destinations[nextId].properties.cartodb_id
      : null;
  },
  ...
  getPreviousDestinationIdSelected: function () {
    ...
    return destinations[previousId]
      ? destinations[previousId].properties.cartodb_id
      : null;
  },

  getRegionsVisited() {
    return this.getSimplifiedDestinationsForSelected()
      .map((destination) =>
        destination.properties.new_region.replace(/ /g, "").toLowerCase()
      )
      .filter(
        (region, pos, self) => self.indexOf(region) == pos && region !== "u.s"
      );
  },
  ...
  getMonthsSelectedWithAngles() {
    return this.monthsForSelected().map((month) => {
      return {
        ...
        endAngle: this.getDateAngle(
          month +
            "-" +
            new Date(2015, parseInt(month.split("-")[1]) - 1, 0).getDate()
        ),
      };
    });
  },
  ...
  getSelectedData() {
    if (this.data.selectedId) {
      return this.getOfficeholderData(
        this.data.selectedId,
        this.data.selectedOffice
      );
    } else {
      return {
        visits: this.getAllDestinations(),
      };
    }
  },

  getOfficeholderStartAngle(id, office) {
    return !id
      ? 0
      : this.getDateAngle(
        this._constrainedDate(
          this.getOfficeholderData(id, office).took_office
        )
      );
  },

  getOfficeholderEndAngle(id, office) {
    return !id
      ? 0
      : this.getDateAngle(
        this._constrainedDate(
          this.getOfficeholderData(id, office).left_office
        )
      );
  },

  getOfficeholderAdjustedStartAngle(id, office) {
    return !id
      ? 0
      : this.getDateAngle(
        this._constrainedDate(this.getOfficeholderData(id, office).start_date)
      );
  },

  getOfficeholderAdjustedEndAngle(id, office) {
    return !id
      ? 0
      : this.getDateAngle(
        this._constrainedDate(this.getOfficeholderData(id, office).end_date)
      );
  },

  getRegionMetadata(slug) {
    return RegionsMetadata.filter((region) => region.slug == slug)[0];
  },

  getTimelineRotation: function () {
    return (
      360 -
      ((this.getOfficeholderEndAngle(
        this.getSelectedId(),
        this.getSelectedOffice()
      ) +
        this.getOfficeholderStartAngle(
          this.getSelectedId(),
          this.getSelectedOffice()
        )) /
        2 /
  ...
  getTimelineRotationRadians: function () {
    return (
      Math.PI * 2 -
      (this.getOfficeholderEndAngle(
        this.getSelectedId(),
  ...
  visitedLocation: function (id, office) {
    ...
    if (this.hasVisibleLocation()) {
      var [lat, lng] = this.getDestinationDetails([
        this.getVisibleLocationIds()[0],
      ])[0].geometry.coordinates;
      destinationIds = this.getSimplifiedDestinationsForOfficeholder(
        id,
        office
      ).filter(
        (v) =>
          v.geometry.coordinates[0] == lat && v.geometry.coordinates[1] == lng
      );
    }
  ...
  getLatLng: function (id) {
    return this.getDestinationDetails([id])[0].geometry.coordinates;
  },
