...
// this outputs formatted data to be stored in `/data/presTerms.json`
...
var parseRawData = function () {
  var pres = PresidentialTerms.map((p) => {
...
var getSimplifiedDestinationsForOfficeholder = function (id, office) {
  return DestinationsJson.features
    .filter(
      (destination) =>
        destination.properties.num == parseInt(id) &&
        destination.properties.position.toLowerCase() == office
    )
    .sort((a, b) =>
      a.properties.date_convert < b.properties.date_convert ? -1 : 1
    );
};
...
var cleanupDestinations = function (id, office) {
  return getSimplifiedDestinationsForOfficeholder(id, office).map((visit) => {
    ...
    visit.geometry.coordinates = visit.geometry.coordinates.map(
...
var getTerm = function (id, office) {
  var terms = office == "president" ? PresidentialTerms : SOSTerms,
    ...
    took_office =
      officeholder.took_office > startDate
        ? officeholder.took_office
        : startDate,
    left_office =
      officeholder.left_office < endDate ? officeholder.left_office : endDate;
...
var getAdjustedTerm = function (id, office) {
  var term = getTerm(id, office),
    ...
    firstDate =
      visits.length > 0 && visits[0].properties.date_convert < term[0]
        ? visits[0].properties.date_convert.substring(0, 10)
        : term[0],
    lastDate =
      visits.length > 0 &&
      visits[visits.length - 1].properties.date_convert > term[1]
        ? visits[visits.length - 1].properties.date_convert.substring(0, 10)
        : term[1];
...
var isValidDate = function (dateString) {
  ...
  if (year < 1904 || year > 2016 || month == 0 || month > 12) return false;
